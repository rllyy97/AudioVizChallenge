...
public class PlayerController implements Initializable {
    ...
    private void handleReady() {
        ...
        lengthText.setText(f.format(duration.toMillis()) + " ms");
        Duration ct = mediaPlayer.getCurrentTime();
        currentText.setText(ct.toString() + " ms");
        currentVisualizer.start(numBands, vizPane);
        timeSlider.setMin(0);
        timeSlider.setMax(duration.toMillis());
        timeSlider.valueChangingProperty().addListener((ObservableValue<? extends Boolean> obs, Boolean wasChanging, Boolean isNowChanging) -> {
            mediaPlayer.pause();
            if (! isNowChanging) {
    ...
    private void handleUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) {
        ...
        currentText.setText(f.format(ms) + " ms");
